...
def replace_const_expression(text: str, name: str, expression: str) -> str:
    pattern = rf"const {re.escape(name)} = .*?;\n"
...
def build_bundle_script(constant_name: str, global_name: str, payload) -> str:
    """Write a deterministic gzip+base64 bundle consumed by the offline UI.

    The dashboard stays completely offline for financial data.  Keeping the
    large generated arrays in separate, compressed scripts also makes the
    generated artifacts safe to transport through repository APIs with small
    request limits.
    """
    ...
    return (
        ...
        f"const {constant_name} = \"{encoded}\";\n"
        "(function (root) {\n"
        "  const binary = atob(" + constant_name + ");\n"
        "  const bytes = Uint8Array.from(binary, (character) => character.charCodeAt(0));\n"
        "  root[\"" + global_name + "\"] = JSON.parse(new TextDecoder(\"utf-8\").decode(pako.ungzip(bytes)));\n"
        "})(window);\n"
    )
...
def build(args: argparse.Namespace) -> None:
    ...
    atomic_write_text(
        EMBEDDED_DATA_PATH,
        build_bundle_script("FURUSATO_DATA_GZIP_B64", "FURUSATO_DATA", data),
    )
    atomic_write_text(
        EMBEDDED_HISTORY_PATH,
        build_bundle_script(
            "FURUSATO_HISTORY_GZIP_B64",
            "FURUSATO_HISTORY",
            {"meta": meta, "history": history},
        ),
    )
    index = replace_const(index, "BASE_AMOUNT_SOURCE_META", base_source_meta)
    index = replace_const_expression(index, "DATA", "window.FURUSATO_DATA")
    index = replace_const_expression(index, "FIVE_YEAR_META", "window.FURUSATO_HISTORY.meta")
    index = replace_const_expression(index, "FIVE_YEAR_HISTORY", "window.FURUSATO_HISTORY.history")
    ...
    script_tags = (
        ...
        '<script src="data/embedded_history.js"></script>\n'
        '<script src="vendor/chart.umd.min.js"></script>'
    )
    index, tag_count = re.subn(
        r'(?:<script src="vendor/pako_inflate\.min\.js"></script>\n'
        r'<script src="data/embedded_data\.js"></script>\n'
        r'<script src="data/embedded_history\.js"></script>\n)?'
        r'<script src="vendor/chart\.umd\.min\.js"></script>',
        script_tags,
        index,
        count=1,
    )
    if tag_count != 1:
        fail("index.html Chart.js script tag not found")
